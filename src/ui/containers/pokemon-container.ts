...
export const POKEMON_CONTAINER_WIDTH = 18;
export const POKEMON_CONTAINER_HEIGHT = 17;
...
export abstract class PokemonContainer extends Label {
  ...
  protected initCommonElements(): void {
    const starterPassiveBg = globalScene.add
      .image(-7, -3.5, "passive_bg")
      .setOrigin(0)
      .setScale(0.75)
      .setVisible(false);
    ...
    for (let i = 0; i < 3; i++) {
      const shinyIcon = globalScene.add
        .image(i * -3 + 3, -6.5, "shiny_star_small")
        .setScale(0.5)
        .setOrigin(0)
        .setVisible(false);
    ...
    this.label = addTextObject(-8, -6.5, "0", TextStyle.WINDOW, { fontSize: "32px" }).setOrigin(0).setVisible(false);
    this.label.setShadowOffset(2, 2);
    this.content.add(this.label);

    this.hiddenAbilityIcon = globalScene.add.image(3, -1.5, "ha_capsule").setOrigin(0).setScale(0.5).setVisible(false);
    this.favoriteIcon = globalScene.add.image(-9, -1.5, "favorite").setOrigin(0).setScale(0.5).setVisible(false);
    this.classicWinIcon = globalScene.add
      .image(-9, 3.5, "champion_ribbon")
      .setOrigin(0)
      .setScale(0.5)
      .setVisible(false);
    this.candyUpgradeIcon = globalScene.add.image(3, 3.5, "candy").setOrigin(0).setScale(0.25).setVisible(false);
    this.candyUpgradeOverlayIcon = globalScene.add
      .image(3, 3.5, "candy_overlay")
      .setOrigin(0)
      .setScale(0.25)
      .setVisible(false);
  ...
  protected updateIcon(species: PokemonSpecies, props: DexAttrProps): void {
    ...
    this.icon = globalScene.add
      .sprite(-11, -6.5, species.getIconAtlasKey(formIndex, shiny, variant))
      .setScale(0.5)
      .setOrigin(0)
