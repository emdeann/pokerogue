...
export class AchvsUiHandler extends MessageUiHandler {
  ...
  setup() {
    ...
    const yOffset = 6;
    this.gridHelper = new ScrollableGridHelper<Phaser.GameObjects.Sprite, GridItem>(0, 0, {
      rows: SHOWN_ROWS,
      columns: COLS,
      scrollBar: {
        x: this.iconsBg.width - 9,
        y: this.iconsBg.y + yOffset,
        width: 4,
        height: this.iconsBg.height - yOffset * 2,
      },
      cells: {
        x: 5,
        y: this.headerBg.height + 8,
        spacingX: 17,
        spacingY: 19,
  ...
  protected showAchv(achv: Achv): void {
    ...
    this.unlockText.setText(unlocked ? new Date(unlocks[achv.id]).toLocaleDateString() : i18next.t("achv:locked.name"));
  }

  /**
   * Detail panel handler to show a voucher.
   * @param achv - The voucher to show
   */
  protected showVoucher(voucher: Voucher): void {
    const unlocks = globalScene.gameData.voucherUnlocks;
    const unlocked = Object.hasOwn(unlocks, voucher.id);
    this.titleText.setText(getVoucherTypeName(voucher.voucherType));
    this.showText(voucher.description);
    this.unlockText.setText(
      unlocked ? new Date(unlocks[voucher.id]).toLocaleDateString() : i18next.t("voucher:locked"),
    );
  }
}
